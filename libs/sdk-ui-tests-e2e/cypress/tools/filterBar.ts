...
import { IOffset } from "../support/commands/dnd";
...
export class AttributeFilter {
    ...
    move(dropName: string, offset: { x?: number; y?: number } = DRAG_POSITION.NEXT): AttributeFilter {
        const testClass = getTestClassByTitle(this.name);
        const dropTestClass = getTestClassByTitle(dropName);
        cy.dragElementTo(
            `.dash-filters-attribute ${testClass}`,
            `.dash-filters-attribute ${dropTestClass}`,
            offset,
        );
        return this;
    }
...
export class FilterBar {
    ...
    addAttributeAtPosition(
        name: string,
        dropName: string,
        offset: IOffset = DRAG_POSITION.NEXT,
    ): AttributeFilter {
        const dropClass = getTestClassByTitle(dropName);
        cy.dragElementTo(NEW_ATTRIBUTE_FILTER_SELECTOR, `.dash-filters-attribute ${dropClass}`, offset);
        return new AttributeFilter(name).search(name).select();
    }
    ...
    hasAttributeFiltersWithValue(attributeFilters: ReadonlyArray<[string, string]>) {
        ...
        attributeFilters.forEach((attributeFilter, index) => {
    ...
    addAttributeAtDateFilter(name: string): AttributeFilter {
        cy.dragElementTo(NEW_ATTRIBUTE_FILTER_SELECTOR, ".dash-filters-date");
        return new AttributeFilter(name).search(name).select();
    }
}
