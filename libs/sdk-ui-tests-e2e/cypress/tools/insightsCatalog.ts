...
export class InsightsCatalog {
    ...
    hasItemsCount(count: number, condition = "") {
        const fullCondition = `have.length${condition ? `.${condition}` : ""}`;
        cy.get(this.getElementSelector(".gd-visualizations-list-item")).should(fullCondition, count);
        return this;
    }
